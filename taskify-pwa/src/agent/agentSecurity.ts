...
const persistentAgentSecurityStore: AgentSecurityStore = {
  get() {
    try {
      const raw = idbKeyValue.getItem(TASKIFY_STORE_TASKS, AGENT_SECURITY_STORAGE_KEY);
      return raw ? normalizeAgentSecurityConfig(JSON.parse(raw)) : defaultAgentSecurityConfig();
    } catch {
...
let currentAgentSecurityStore: AgentSecurityStore = persistentAgentSecurityStore;
...
export function setAgentSecurityStore(store: AgentSecurityStore | null | undefined): void {
  currentAgentSecurityStore = store ?? persistentAgentSecurityStore;
}
