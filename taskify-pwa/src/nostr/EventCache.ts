...
/**
 * Lightweight in-memory cache to avoid re-processing duplicate events.
 * This can be expanded later to persist or shard if needed.
 */
export class EventCache {
  ...
  add(event: NostrEvent): void {
    ...
    if (this.seenIds.size > this.maxSize) {
      // Simple eviction: remove the oldest inserted id.
